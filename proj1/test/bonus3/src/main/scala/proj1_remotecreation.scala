...
class CreatApplication(host: String, port: Int) extends Bootable{
  val address = Address("akka.tcp", "CreatApplication", host, port)
...
//  val system = ActorSystem("sys") 
  ...
  val localActor = system.actorOf(Props(classOf[CreationActor]), name = "creationActor")
...
class SuperBoss(range: Long, len: Long, subSysNum: Long) {
      val hosts = new Array[String](10)
      val ports = new Array[Int](10)
      var ipcount: Int = 0
      hosts(0) = "127.0.0.1"   ;     ports(0) = 2552 ; 
      hosts(1) = "127.0.0.1"   ;     ports(1) = 2553 ; 
      ...
      if(range < subSysNum)
        ...
        for(cons <-1l to (len/2l))
         ...
         endConstant -= (len/2l)*(len/2l) 
          while(start<=range){
             val app = new CreatApplication(hosts(ipcount), ports(ipcount))
...
object proj1_remotecreation{
  def main(args: Array[String]) : Unit = {
      var superBoss = new SuperBoss(40l,24l,5l)
    }
}
